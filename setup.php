...
function getSetting($key, $default = '') {
    return (string) $_SESSION['wikidocs']['setup'][$key] ?? (string)$default;
}
...
if (file_exists("datasets/config.inc.php")) {
    die("Wiki|Docs is already configured.");
}
...
if($g_act=="preliminary_check") {	
	...
	if (function_exists('apache_get_modules')) {
		...
		foreach ($required_modules as $module) {
			$installed[$module] = in_array($module, apache_get_modules());
		}
	}

	$versions = [];
	foreach (['Core','dom','json','mbstring','session','xml'] as $extension) {
	...
	<div class="row">
		...
		if($g_act=="preliminary_check"){
			...
			<div class="col s12">
				...
				<?php foreach ($versions as $key => $extension): ?>
                    <li class="collection-item">
		                        <span class="secondary-content">
                            <i class="material-icons <?= $extension ? 'green-text' : 'red-text' ?>">
                                <?= $extension ? 'check_circle' : 'cancel' ?>
                            </i>
                        </span>
						<?=$key?>: <?= $versions[$key] ?> 
                    </li>
